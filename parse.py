...
class Zone:
    ...
    def __init__(
        ...
        self.drones: Dict[str, bool] = drones
    ...
    def process_metadata(cls, name: str, x: int, y: int,
        ...
        if zone_type not in ['normal', 'blocked', 'restricted', 'priority']:
            raise MetadataError(
                f"Line {line_count}. Unknown zone type: '{zone_type}'")
        if max_drones <= 0 and zone_type != 'blocked':
            raise InvalidDroneNumberError("Max drones must be at least 1")
            ...
            if ':' in line:
                ...
                if key in ('hub', 'start_hub', 'end_hub'):
                    ...
                    if data[0] in map.n_zones:
                        raise MapParseError(
                            f"Line {line_count}. Duplicate zone: {data[0]}")
                    ...
                    if (x, y) in map.zones:
                        raise MapParseError(
                            f"Line {line_count}."
                            f" Duplicate coordinates: {data[0]}")
                    zone = Zone.process_metadata(
                        data[0], x, y, metadata, map, line_count)
                    map.add_zone(zone)
                    if key == 'start_hub':
                        if map.start:
                            raise MapParseError(
                                f"Line {line_count}."
                                f" Map must have only 1 start_hub")
                        map.start = zone
                        i = 1
                        ...
                        if map.end:
                            raise MapParseError(
                                f"Line {line_count}."
                                f" Map must have only 1 end_hub")
                        map.end = zone
                    ...
                    if zone_a in map.n_zones and zone_b in map.n_zones:
                        map.connections.setdefault(
                            zone_a, {})[zone_b] = max_link
                    else:
